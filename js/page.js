...
function openPage(pageId,data){
    ...
    if(headerD && headerD.freeze){
        header.addClass('freeze');
    }else{
        header.removeClass('freeze');
    }
    if(footerD && footerD.freeze){
...
$('body').on('click', bodyClick)
	    ...
	    .on('dblclick','.mask_body_x',maskBodyXDblClick)
	    .on('dblclick','.mui-bar.freeze',goInFreezeBar)
	    .on('dblclick','.mask_bar',goOutFreezeBar)
	    .on('dblclick',bodyDblClick);  
function goOutAll(){
	bodyDblClick();
	if(maskIdx > 22 || $('.mask_bar')[0]){	
    	goOutAll();
    }
}
function bodyDblClick(){
    if(maskIdx > 22){	//退出进入编辑的冰冻组件
    	goOutFreezeComp.call($('#root_'+maskIdx)[0]);
    }
    var mask_bar = $('.mask_bar')[0];
    if(mask_bar){	//退出进入编辑的冰冻顶部栏或底部栏
    	goOutFreezeBar.call(mask_bar);
    }
}
function goInFreezeBar(e){
	if(e){
		e.stopPropagation();
	}
	focusBar($(this));
...
function focusBar(bar){
	var mask = $('<div class="mask_bar">');
	$('.mui-inner-wrap').append(mask);
	$('.mui-bar').not(bar).css('z-index',995);
	bar.addClass('focus_bar');
}
function unFocusBar(){
	$('.mask_bar').remove();
	$('.mui-bar').css('z-index',997);
	$('.focus_bar').removeClass('focus_bar');
}
function goOutFreezeBar(e){
	if(e){
		e.stopPropagation();
	}
	$('.focus_bar').addClass('freeze');
	unFocusBar();
}
...
function goInFreezeComp(e){
	...
	if(rootId=='root_0'){
		if($('.mask_body_x').length==0){
			$('.mui-bar').append('<div class="mask_body_x"></div>');
		}
	}else if(rootId=='root_1' || rootId=='root_4'){
		if(!obj.is('.focus_bar')){
			focusBar(obj);
		}
	}
...
function checkDragLimit(){
    ...
    if(limit.length>0){
        for(var i=0;i<limit.length;i++){
        	var limitType = limit[i];
            if(limitType=='header' || limitType=='footer'){
            	$(limitType).addClass('dragLimit');
            }else{
            	$('.comp_'+limitType).addClass('dragLimit');
            }
        }
    }
}
...
function dragover() {
    ...
    while(!obj.is('body')){
        ...
        if(!cont && obj.is('.container') && !obj.is('.freeze, .dragLimit')){
            cont = obj;
        }
        obj = obj.parent();
    }
...
function drop() {
    if(parent.dragingType == 'component'){
        ...
        compTitleWidth();
        $('.dragFocus').removeClass('dragFocus');
        $('.dragLimit').removeClass('dragLimit');
